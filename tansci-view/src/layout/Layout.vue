        ...
        <el-aside :style="{ height: defaultHeight + 'px' }" :width="asideWidth" style="padding-bottom:4rem;">
            ...
            <el-menu router :default-active="$route.path" :collapse="isCollapse" @select="onSelect" text-color="#242e42"
                ...
                <template v-for="item in routers" :key="item">
                    <el-menu-item v-if="!item.children || item.children.length == 1 || item.englishName === 'list'"
                        :index="item.type == 1 ? item.path : ''">
                        ...
                        <span v-else-if="item.type == 2" @click="onNestedLink(item)" style="vertical-align: middle;">{{
                            item.chineseName }}</span>
                        <a v-else-if="item.type == 3" :href='item.url' target='_blank'
                            style="vertical-align: middle;text-decoration: none;color:#242e42;">{{ item.chineseName
                            }}</a>
                    </el-menu-item>
            ...
            <el-header height="50">
                <div class="header">
                    ...
                    <div style="padding-right:0.4rem;">
                        ...
                        <el-dialog v-model="dialogPass" title="修改密码" width="35%" :destroy-on-close="true"
                            :show-close="false">
                            <el-form :model="passForm" :rules="rules" ref="validateForm" status-icon label-width="100px">
                                <el-form-item label="原始密码" prop="oldPassword"
                                    ...
                                    <el-input class="input-size" type="password" v-model="passForm.oldPassword"
                                        prefix-icon="Lock" autocomplete="off" style="width:100%" />
                                </el-form-item>
            ...
            <el-main class="flex-1">
                <router-view>
                    <template #default="{ Component, route }">
                        <keep-alive>
                            <component style="margin: 0.4rem;" :is="Component" :key="route.fullPath" />
                        </keep-alive>
                    </template>
                </router-view>
                <!-- <keep-alive>
...
const state = reactive({
    isCollapse: localStorage.getItem('isMobile') || false,
    asideWidth: localStorage.getItem('isMobile') ? '44px' : '240px',
    defaultHeight: null,
    routers: [],
    dialogPass: false,
    passForm: {
