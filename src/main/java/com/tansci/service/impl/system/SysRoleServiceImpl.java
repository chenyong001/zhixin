...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
  ...
  public IPage<SysRole> page(Page page, SysRole role) {
    ...
    if (Objects.equals(0, SecurityUserUtils.getUser().getType())) {
      ...
      List<SysUserOrg> orgs = sysUserOrgService
          .list(Wrappers.<SysUserOrg>lambdaQuery().in(SysUserOrg::getOrgId, SecurityUserUtils.getUser().getOrgIds()));
      ...
      queryWrapper = Wrappers.<SysRole>lambdaQuery().in(SysRole::getCreator, userIds)
          .like(Objects.nonNull(role.getName()), SysRole::getName, role.getName())
          .orderByDesc(SysRole::getCreateTime, SysRole::getUpdateTime);
    }
    IPage<SysRole> roleIPage = this.baseMapper.selectPage(page, queryWrapper);

    List<SysDic> statusList = roleIPage.getSize() > 0 ?
        sysDicService.list(Wrappers.<SysDic>lambdaQuery().eq(SysDic::getGroupName, "role_status")) :
        new ArrayList<>();
  ...
  public List<SysMenuRoleVo> menuRoleList(SysMenuRole role) {
    ...
    if (!Objects.equals(0, SecurityUserUtils.getUser().getType())) {
      ...
      if (Objects.nonNull(roleVos)) {
        // 获取所有菜单的父id
        List<Integer> parentIds = roleVos.stream().map(SysMenuRoleVo::getParentId).distinct()
        ...
        if (Objects.nonNull(parentIds) && parentIds.size() > 0) {
          ...
          parentList.forEach(item -> {
            menuRoleVos.add(SysMenuRoleVo.builder().id(item.getId()).parentId(item.getParentId()).name(item.getName())
                .chineseName(item.getChineseName()).sort(item.getSort()).roleId(null).menuId(null).build());
          });
    ...
    Map<Integer, List<SysMenuRoleVo>> map = roleVos.stream()
        .collect(Collectors.groupingBy(SysMenuRoleVo::getParentId, Collectors.toList()));
